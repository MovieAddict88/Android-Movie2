...
if (!isLoggedIn()) {
    ...
    $stmt = $pdo->prepare("SELECT b.*, c.brand, c.model, c.image, p.status as payment_status 
                           FROM bookings b 
                           JOIN cars c ON b.car_id = c.id 
                           LEFT JOIN (SELECT booking_id, status FROM payments ORDER BY created_at DESC LIMIT 1) p ON b.id = p.booking_id 
                           WHERE b.user_id = ? 
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div class="flex-1">
                            ...
                            <?php if ($booking['payment_status']): ?>
                                <p class="text-xs mt-2 italic text-gray-500">Payment Status: <?= ucfirst($booking['payment_status']) ?></p>
                            <?php endif; ?>
                        </div>
                        <div class="text-right flex flex-col items-end gap-2">
                            ...
                            <?php if ($booking['status'] === 'pending' && $booking['payment_status'] !== 'pending' && $booking['payment_status'] !== 'approved'): ?>
                                <a href="payment.php?booking_id=<?= $booking['id'] ?>" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition text-sm">Pay Now</a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
